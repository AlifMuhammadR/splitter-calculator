...
<div id="sidebar" class="position-fixed sidebar-hidden">
    <div class="inner p-3">
        <!-- konten sidebar -->
        <a href="{{ route('lab') }}" class="btn w-100 text-white fw-bold" style="background: linear-gradient(87deg, #627594 0, #8898aa 100%);
           border-radius: 25px;
           border: none;
           pointer-events: none;">
        ...
        <div class="mb-2">
            ...
            <input type="number" id="input-power" class="form-control form-control-sm mb-2" oninput="updateOLTPower()" value="7">
        ...
        <div class="d-grid gap-2 mb-2">
            <button class="btn btn-sm btn-light w-100 fw-bold" onclick="gatherAndSaveTopology()"><i class="bi bi-bookmark-plus-fill"></i> Save Topology</button>
        ...
        <div class="d-grid gap-2 mb-2">
            <button class="btn btn-sm" style="background: linear-gradient(87deg, #2d93ce 0, #107abc 100%); border: none;" onclick="exportTopology()">⬇️ Export (.json)</button>
            <input type="file" id="import-file" accept=".json" class="form-control form-control-sm" onchange="importTopology(this.files[0])">
        </div>
        <div class="divider my-3"></div>
        <a href="{{ route('lab') }}" class="btn w-100 text-white fw-bold mb-2" style="background: linear-gradient(87deg, #627594 0, #8898aa 100%);border-radius: 25px; border: none;"><i class="bi bi-x-circle"></i> Keluar</a>
...
<!-- Canvas Full Area -->
<div id="map-canvas" data-lab-id="{{ $lab['id'] }}" style="position: relative; height: 100vh; background: #f8fafc;">
</div>
...
<div id="info-card" class="card shadow-sm border border-info d-none" style="position: fixed; top: 360px; right: 20px; min-width: 250px;transition: all 0.3s ease-in-out; z-index: 1;">
    <div class="card-body">
...
<!-- Button toggle -->
<button onclick="toggleStatusTable()" class="btn fw-bold" style="background: linear-gradient(87deg, #2dce89 0, #10BC69 100%); position: fixed; bottom: 20px; right: 20px; border-radius: 25px; color: #323233;">
    <i class="bi bi-table"></i> Tabel Status
</button>

<!-- Tabel Status Power Loss -->
<div id="status-table-box" class="bg-white border rounded shadow-lg p-3 mt-3" style="width: 350px;display: none; animation: float 2s ease-in-out infinite; z-index: 2;">
...
<script>
    ...
    jsPlumb.ready(function() {
        jsPlumb.setContainer("map-canvas");

        // Helper: robust ambil type node
        function getNodeType(node) {
            return node.dataset.type || (node.querySelector('strong') ? node.querySelector('strong').innerText : '') || '';
        }
        ...
        window.addNode = function(type) {
            ...
            el.innerHTML = `
        <button class="btn btn-danger btn-sm btn-delete-node" style="position: absolute; top: -8px; right: -8px; z-index: 2; border-radius: 50%; width: 22px; height: 22px; padding: 0; font-size: 14px; line-height: 1;" title="Hapus Node">&times;</button>
        <strong>${label}</strong>
        <div class="output-power" style="font-size: 12px; color: green;">${powerDisplay}</div>
    `;
            ...
            // Tombol hapus node
            el.querySelector('.btn-delete-node').onclick = function(e) {
                e.stopPropagation();
                Swal.fire({
                    title: 'Hapus node ini?',
                    text: 'Semua kabel yang terhubung ke node juga akan dihapus.',
                    icon: 'warning',
                    showCancelButton: true,
            ...
            el.onclick = function(e) {
                if (e.target.classList.contains("jsplumb-endpoint")) return;
                if (e.target.classList.contains("btn-delete-node")) return; // biar klik tombol hapus tidak trigger connect
                if (!selectedNode) {
                    selectedNode = el;
            ...
            // ENDPOINTS (sesuaikan splitter)
            let endpointCount = 1;
            let anchors = ["Top", "Right", "Bottom", "Left"];
            if (type.startsWith('Splitter')) {
                let split = type.split(' ')[1];
                let nPorts = parseInt(split.split(':')[1]) || 4;
                endpointCount = nPorts;
                anchors = [];
                for (let i = 0; i < nPorts; i++) {
                    anchors.push([(i + 1) / (nPorts + 1), 1, 0, 1]);
                }
            }
        ...
        jsPlumb.bind("connection", function(info) {
            ...
            let meta = conn._customMeta || {};
            let lossCable = meta.loss || 0;
            let color = meta.color || "black";
            ...
            let fromPower = parseFloat(source.dataset.power || document.getElementById("input-power").value || 0);
            let powerRx = fromPower - lossCable;
            if (target.querySelector(".output-power")) target.querySelector(".output-power").innerText = `${powerRx.toFixed(2)} dB`;
        ...
        function connectNodeElements(source, target, length) {
            ...
            if (selectedCableName === "Patchcord") paint.dashstyle = "4 2";
        ...
        window.resetTopology = function() {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    resetMap();
                    Swal.fire('Topologi berhasil direset!', '', 'success');
                }
            });
        };

        // ADD NODE DARI DB/IMPORT/LOAD
        ...
        <div class="output-power" style="font-size: 12px; color: green;">${parseFloat(node.power).toFixed(2)} dB</div>
    `;
            ...
            let endpointCount = 4;
            let anchors = ["Top", "Right", "Bottom", "Left"];
            if (node.type.startsWith('Splitter')) {
                let split = node.type.split(' ')[1];
                let nPorts = parseInt(split.split(':')[1]) || 4;
                endpointCount = nPorts;
                anchors = [];
                for (let i = 0; i < nPorts; i++) {
        ...
        window.drawConnectionFromDB = function(conn) {
            ...
            jsConn.bind("contextmenu", function(conn, e) {
                e.preventDefault();
                if (confirm('Hapus kabel ini?')) {
                    jsPlumb.deleteConnection(conn);
                    lines = lines.filter(l => l.conn !== conn);
        ...
        window.calculateAllLoss = function() {
            lines.forEach(conn => {
                ...
                const sourcePower = parseFloat(from.dataset.power || document.getElementById("input-power").value || 0);
                const powerRx = sourcePower - loss;
                if (to.querySelector(".output-power")) to.querySelector(".output-power").innerText = `${powerRx.toFixed(2)} dB`;
            });
        };

        window.loadTopology = function(id) {
            fetch(`/topologi/load/${id}`)
                .then(res => res.json())
                .then(data => {
                    if (!data.nodes || !data.connections) return;
                    resetMap();
                    data.nodes.filter(n => n && n.id && n.type)
                        .forEach(node => {
        ...
        window.exportTopology = function() {
            ...
            document.querySelectorAll("#map-canvas > div").forEach(node => {
                if (!node.id || !getNodeType(node)) return;
                topology.nodes.push({
                    id: node.id,
        ...
        window.gatherAndSaveTopology = function() {
            const topology = {
                nodes: [],
                connections: [],
                power: parseFloat(document.getElementById('input-power')?.value || 0),
        ...
        async function saveTopology(topology, labId) {
            ...
            try {
                await fetch(`/topologi/save/${labId}`, {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
